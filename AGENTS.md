...
7. `assemble_package()`: fresh copy of committed `template/` → `package/`, port META-INF overwrites `META-INF/com/android/`, `super.img` split into exactly 54 chunks `images/super.img.0..53` (chunk = ceil(size/54), names unpadded — the install scripts loop `$i` 0..53, `split(1)` zero-pads and does NOT match).
8. `create_recovery_zip()`: packs `package/` into `HyperOS-port-duchamp-<ver>.zip` with DEFLATE-9 (max). `META-INF` updater-script + ARM update-binary (already in `template/`) make it flashable in TWRP/OrangeFox; fastboot users unzip and run the install scripts. CI uploads both `super.img` and the ZIP.
